...
import { OrderHistory as OrderHistoryType } from "@/types/order"
        ...
        <div className="rounded-lg border border-gray-200 p-4">
            <div className="flex flex-row pb-2">
                <div className="flex flex-grow flex-col ">
                    <Link className="text-ellipsis text-xl font-bold" href={`/store/${history._id}`}>
                        {history.store.name}
                    </Link>
                    <div className="pb-1 text-sm text-gray-500">{new Date(history.timestamp).toLocaleString('ko')}</div>
                    <div>{getOrderStatus(history.status)}</div>
                </div>
                {history?.store?.images?.length &&( 
                <div>
                    <img src={history.store.images?.[0]}
                         className="h-20 w-20" 
            ...
            <Link href={`/order/${history._id}/review`}
